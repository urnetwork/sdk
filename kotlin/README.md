# URnetwork SDK for Kotlin

Default installation after the first registry publication:

```text
Gradle: implementation("io.ur:urnetwork-sdk:<version>")
```

Build a local package with `make -C sdk/kotlin` and verify it with
`make -C sdk/kotlin check-package`. Native desktop builds reuse the CGo module;
...
[language examples](https://github.com/urnetwork/examples/tree/main/kotlin).
