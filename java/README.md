# URnetwork SDK for Java

Default installation after the first registry publication:

```text
Maven/Gradle: io.ur:urnetwork-sdk:<version>
```

Build a local package with `make -C sdk/java` and verify it with
`make -C sdk/java check-package`. Native desktop builds reuse the CGo module;
...
[language examples](https://github.com/urnetwork/examples/tree/main/java).
