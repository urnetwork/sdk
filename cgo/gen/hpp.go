...
// Only ProvideSecretKey contains an intentionally raw-byte string. Keep its
// public field unchanged; the alternate field exists solely in this JSON codec.
const hppProvideSecretKeyJson = `namespace detail {
	...
	for _, name := range order {
		...
		if name == "ProvideSecretKey" {
			if len(fields) != 2 ||
				...
				return fmt.Errorf("ProvideSecretKey JSON model changed")
			}
			b.WriteString(hppProvideSecretKeyJson)
			continue
		}
		fmt.Fprintf(b, "inline void to_json(nlohmann::json& j, const %s& v) {\n\tj = nlohmann::json::object();\n", name)
		for _, f := range fields {
